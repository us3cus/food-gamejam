...
const VERSION_URL: StringName = "https://godotengine.org/asset-library/api/asset/3901"
...
func set_defaults() -> void:
	downloading.value = 0
	installed_label.text = "Installed version %s" % SteamServer.get_godotsteam_version()
...
func check_for_updates() -> void:
	if not ProjectSettings.get_setting("steam/settings/godotsteam_server/check_for_updates"):
		return
	...
	if http_request.request(VERSION_URL) != OK:
		printerr("Failed to request GodotSteam Server plug-in remote current version")
...
func _on_http_request_completed(result: int, _response_code: int, _headers: PackedStringArray, body: PackedByteArray) -> void:
	...
	if convert_version(new_version) > convert_version(SteamServer.get_godotsteam_version()):
		print("New GodotSteam Server version %s available" % new_version)
...
func _on_install_pressed() -> void:
	print("Removing older GodotSteam Server %s plug-in" % SteamServer.get_godotsteam_version())
	OS.move_to_trash(ProjectSettings.globalize_path("res://addons/godotsteam_server"))
...
func _on_download_request_completed(result: int, _response_code: int, _headers: PackedStringArray, body: PackedByteArray) -> void:
	...
	if result != HTTPRequest.RESULT_SUCCESS:
		printerr("Failed to download new GodotSteam Server version %s" % result)
		update_button.disabled = false
		return
